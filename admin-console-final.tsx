    ...
    <div className="space-y-5">
        ...
        <div className="border-b border-border bg-gradient-to-r from-accent/10 via-background to-background p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="rounded-xl border border-border bg-card px-4 py-3 text-right">
              ...
              <div className="text-xs text-muted-foreground">Connected to the Admin settings system</div>
            </div>
          </div>
        </div>
      ...
      {section === "rules" && (
        <div className="grid gap-4 lg:grid-cols-2">
          <section className="surface-panel p-5"><div className="text-[10px] font-bold uppercase tracking-wider text-accent">Access Policy</div><h3 className="mt-1 font-display text-xl font-bold">Download Rules</h3><p className="mt-1 text-sm text-muted-foreground">Central controls for free/paid downloads.</p><div className="mt-5 space-y-4"><label className="flex items-center justify-between rounded-lg border border-border p-4"><span><span className="block text-sm font-semibold">Login required</span><span className="text-xs text-muted-foreground">Users must sign in before downloading.</span></span><input type="checkbox" checked={loginRequired} onChange={(e) => setLoginRequired(e.target.checked)} /></label><div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-1"><span className={labelCls}>PDF free downloads / day</span><input className={fieldCls} type="number" min="0" value={pdfDaily} onChange={(e) => setPdfDaily(e.target.value)} /></label><label className="grid gap-1"><span className={labelCls}>Excel free downloads / day</span><input className={fieldCls} type="number" min="0" value={excelDaily} onChange={(e) => setExcelDaily(e.target.value)} /></label></div><button type="button" onClick={() => toast.info("Download rules saved successfully.")} className="rounded-md bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground">Save rules</button></div></section>
      ...
      {section === "orders" && (
        <section className="surface-panel p-5"><div className="flex items-center justify-between"><div><div className="text-[10px] font-bold uppercase tracking-wider text-accent">Payments</div><h3 className="mt-1 font-display text-xl font-bold">Orders</h3><p className="mt-1 text-sm text-muted-foreground">Payment/order workspace ready for Live gateway connection.</p></div><StatusBadge status="Not connected" /></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead><tr className="border-b border-border text-xs text-muted-foreground"><th className="pb-3">Order ID</th><th className="pb-3">User</th><th className="pb-3">Product</th><th className="pb-3">Amount</th><th className="pb-3">Status</th><th className="pb-3">Date</th></tr></thead><tbody><tr><td colSpan={6} className="py-12 text-center text-sm text-muted-foreground">No orders yet — payment gateway will populate this table in Live.</td></tr></tbody></table></div></section>
      )}
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-xl border border-border bg-card p-4 shadow-sm"><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">PDF downloads</div><div className="mt-2 text-2xl font-bold tracking-tight">—</div><div className="mt-2 text-[11px] text-muted-foreground">Connects in Live</div></div>
          <div className="rounded-xl border border-border bg-card p-4 shadow-sm"><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Revenue</div><div className="mt-2 text-2xl font-bold tracking-tight">₹—</div><div className="mt-2 text-[11px] text-muted-foreground">Connects in Live</div></div>
        </div>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          ...
          <ul className="mt-4 space-y-2 text-xs text-muted-foreground">
            ...
            <li className="rounded-lg bg-amber-500/5 px-3 py-2">Connect analytics and revenue data in Live.</li>
          </ul>
        </div>
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          ...
          <p className="mt-2 text-xs leading-5 text-muted-foreground">The UI never grants admin access by itself. Live will enforce the same permissions at Supabase RLS/database level.</p>
...
/* ============================================================
   ...
   Phase 1: presentation/workflow only. Backend/RLS connects in Live.
============================================================ */
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              ...
              Database authorization and RLS will be connected in Live.
            </p>
          </div>
      ...
      {section === "users" && (
        <section className="rounded-2xl border border-border bg-card shadow-sm">
          <div className="flex flex-col gap-3 border-b border-border p-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <button
              type="button"
              onClick={() => toast.info("User creation will be connected in Live.")}
          ...
          {roles.map((role) => (
            <div key={role.name} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  type="button"
                  onClick={() => toast.info("Role permissions will be connected in Live.")}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-border bg-card p-5 shadow-xl">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button type="button" onClick={() => toast.info("Status changes connect in Live.")} className="rounded-lg border border-border px-3 py-2 text-xs font-semibold">
                Disable / Enable
              </button>
              <button type="button" onClick={() => toast.info("Role changes connect in Live.")} className="rounded-lg border border-border px-3 py-2 text-xs font-semibold">
                Change role
              </button>
              <button type="button" onClick={() => toast.info("User deletion connects in Live.")} className="rounded-lg bg-destructive px-3 py-2 text-xs font-semibold text-destructive-foreground">
                Delete user
              </button>
            </div>
          </div>
...
function AnalyticsPanel() {
  ...
  const metrics = [
    ...
    ["Sessions", "—", "Connects in Live"],
    ["Page views", "—", "Connects in Live"],
...
function AdsensePanel() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 lg:grid-cols-[1fr_1fr]">
      <div className="surface-panel p-5"><h3 className="font-semibold">Publisher settings</h3><div className="mt-4 space-y-3"><label className="block"><span className="text-xs font-medium text-muted-foreground">Publisher ID</span><input value={publisherId} onChange={e=>setPublisherId(e.target.value)} placeholder="ca-pub-xxxxxxxxxxxxxxxx" className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" /></label><label className="flex items-center justify-between rounded-lg border border-border p-3 text-sm"><span>Auto ads</span><input type="checkbox" checked={autoAds} onChange={e=>setAutoAds(e.target.checked)} /></label><label className="flex items-center justify-between rounded-lg border border-border p-3 text-sm"><span>Test mode</span><input type="checkbox" checked={testMode} onChange={e=>setTestMode(e.target.checked)} /></label><button type="button" onClick={()=>toast.success("AdSense UI settings ready for Live connection.")} className="w-full rounded-lg bg-accent px-4 py-2.5 text-xs font-semibold text-accent-foreground">Save configuration</button></div></div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
        <section className="surface-panel p-5 sm:p-6">
          <div className="flex items-start justify-between gap-3">
            ...
            <button type="button" onClick={() => toast.success("App configuration saved for Live connection.")} className="inline-flex items-center gap-2 rounded-lg bg-accent px-4 py-2.5 text-xs font-semibold text-accent-foreground">
        ...
        <section className="surface-panel p-5 sm:p-6">
          ...
          <div className="mt-5 rounded-xl border border-dashed border-border bg-secondary/30 p-4 text-xs text-muted-foreground">
            <div className="font-semibold text-foreground">Live connection</div>
    ...
    <div className="space-y-6">
      ...
      <section className="surface-panel p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <button type="button" onClick={() => toast.success("Manual backup request queued for Live connection.")} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">Create backup</button>
        </div>
        ...
        <div className="mt-5 rounded-xl border border-dashed border-border bg-secondary/30 p-4 text-xs text-muted-foreground"><strong className="text-foreground">Live:</strong> actual Supabase/database backup, storage snapshots and restore operations will be connected after UI approval.</div>
      </section>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="surface-panel p-5"><h3 className="font-display text-lg font-bold">Maintenance</h3><p className="mt-1 text-sm text-muted-foreground">Control the public maintenance state from the system settings workflow.</p><label className="mt-5 flex items-center justify-between rounded-xl border border-border p-4"><div><div className="text-sm font-semibold">Maintenance mode</div><div className="text-xs text-muted-foreground">Keep disabled during normal operation.</div></div><input type="checkbox" onChange={e=>toast.info(e.target.checked ? "Maintenance mode marked for Live." : "Maintenance mode disabled.")} /></label></section>
      </div>

      <div className="rounded-xl border border-dashed border-border bg-secondary/30 p-4 text-xs text-muted-foreground"><strong className="text-foreground">Live:</strong> real health probes, database latency, storage checks, API monitoring and error telemetry will connect here.</div>
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <section className="surface-panel p-5">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><h3 className="font-display text-xl font-bold">Test delivery</h3><p className="mt-1 text-sm text-muted-foreground">Phase 1 only: UI action. Actual provider delivery connects in Live.</p></div>
      ...
      <section className="surface-panel overflow-hidden">
        ...
        <div className="divide-y divide-border">{templates.map(([name, desc, active]) => <div key={name} className="flex flex-wrap items-center justify-between gap-3 p-4"><div><div className="font-semibold">{name} email</div><div className="text-sm text-muted-foreground">{desc}</div></div><div className="flex items-center gap-3"><StatusBadge status={active ? "Active" : "Disabled"} /><button type="button" onClick={() => toast.info(`${name} template editor will connect in Live.`)} className="rounded-md border border-border px-3 py-2 text-xs font-semibold">Edit</button></div></div>)}</div>
      </section>
    </div>
  );
}
...
function GeneralSettingsPanel() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end"><button type="button" onClick={() => toast.success("General settings saved for Live connection.")} className="rounded-md bg-accent px-5 py-2.5 text-sm font-semibold text-accent-foreground">Save settings</button></div>
    </div>
  );
}
