...
function ToolsPanel() {
  ...
  >({});
  const [cmsToolOverrides, setCmsToolOverrides] = useState<Record<string, any>>({});
  ...
  const loadCmsToolOverrides = async () => {
    try {
      const rows = await getCmsToolOverrides();
      const result: Record<string, any> = {};
      rows.forEach((row: any) => {
        result[row.tool_slug] = row;
      });
      setCmsToolOverrides(result);
    } catch (error) {
      console.warn("CMS tool overrides could not be loaded; using legacy settings.", error);
    }
  };

  useEffect(() => {
    void Promise.all([loadCustomTools(), loadToolConfigs(), loadCmsToolOverrides()]);
  }, []);
  ...
  const allTools: ToolRecord[] = useMemo(() => {
    const builtIn: ToolRecord[] = TOOLS.map((tool) => {
      ...
      const cms = cmsToolOverrides[tool.slug];
      const cmsData =
        cms?.override_data && typeof cms.override_data === "object"
          ? cms.override_data as Record<string, unknown>
          : {};
  ...
  async function togglePublish(tool: ToolRecord) {
    ...
    if (tool.custom) {
      ...
      try {
        const cms = cmsToolOverrides[tool.slug];
        const enabled = nextStatus === "Published" ? true : false;

        await saveCmsToolOverride({
          toolSlug: tool.slug,
          enabled,
          featured: tool.featured,
          displayOrder: cms?.display_order ?? null,
          overrideData: {
            name: tool.name,
          ...
          .upsert(
            {
              ...
              value: {
                ...
                longDescription: existing.longDescription || tool.longDescription,
...
function PostsPanel() {
  ...
  async function savePost(forcePublished?: boolean) {
    ...
    const payload = {
      ...
      meta_description: seoDescription.trim() || null,
      category: category.trim() || "General",
      tags: tags
        .split(",")
        .map((tag) => tag.trim())
