...
function SeoWorkspace({ section }: { section: "seo" | "keywords" | "content-quality" | "internal-links" | "sitemap" | "redirects" }) {
  const [range, setRange] = useState("30 days");
  const configs = {
    seo: { kicker: "SEO CONTROL CENTER", title: "SEO Dashboard", desc: "Monitor technical health, metadata coverage and search readiness.", cards: [["Indexed pages","—","Connect Search Console"],["SEO health","—","Run site audit"],["Missing meta","—","Needs attention"],["Broken URLs","—","From 404 logs"]] },
    keywords: { kicker: "SEARCH VISIBILITY", title: "Keyword Rank Tracker", desc: "Track target keywords, positions, movement and landing pages.", cards: [["Tracked keywords","—","Connect rank source"],["Improving","—","Position gains"],["Declining","—","Needs review"],["Top 10","—","High visibility"]] },
    "content-quality": { kicker: "CONTENT QUALITY", title: "Content Quality", desc: "Review content readiness before publishing and identify optimization gaps.", cards: [["Pages reviewed","—","Content inventory"],["Strong","—","Ready to publish"],["Needs work","—","Optimization queue"],["Missing SEO","—","Metadata gaps"]] },
    "internal-links": { kicker: "SITE STRUCTURE", title: "Internal Links", desc: "Manage internal linking opportunities and identify broken or weak connections.", cards: [["Internal links","—","Site crawl"],["Broken links","—","Needs fixing"],["Orphan pages","—","No incoming links"],["Opportunities","—","Suggested links"]] },
    sitemap: { kicker: "TECHNICAL SEO", title: "Sitemap & Robots", desc: "Control sitemap generation, robots directives and search-engine access.", cards: [["Sitemap","Ready","Generation status"],["URLs","—","Included pages"],["Robots","Configured","Directive status"],["Warnings","—","Validation"]] },
    redirects: { kicker: "URL MANAGEMENT", title: "Redirects & 404", desc: "Manage redirects and turn real 404 traffic into actionable fixes.", cards: [["404 hits","—","Recent traffic"],["Active redirects","—","301/302 rules"],["Broken targets","—","Needs fixing"],["Resolved","—","Fixed URLs"]] },
  } as const;
  const c=configs[section];
  return <div className="space-y-6">
    <section className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div><div className="text-[10px] font-bold uppercase tracking-[0.2em] text-accent">{c.kicker}</div><h2 className="mt-1 font-display text-2xl font-bold tracking-tight">{c.title}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{c.desc}</p></div>
        <select className={fieldCls+" w-auto"} value={range} onChange={e=>setRange(e.target.value)}><option>7 days</option><option>30 days</option><option>90 days</option></select>
      </div>
      <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{c.cards.map(([label,value,note])=><div key={label} className="rounded-xl border border-border bg-background p-4"><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">{label}</div><div className="mt-2 text-2xl font-bold">{value}</div><div className="mt-1 text-[11px] text-muted-foreground">{note}</div></div>)}</div>
    </section>
    {section==="seo" && <div className="grid gap-4 lg:grid-cols-[1.3fr_.7fr]"><section className="surface-panel p-5"><h3 className="font-display font-bold">SEO Health Overview</h3><div className="mt-4 h-52 rounded-xl border border-dashed border-border bg-secondary/30 p-5"><div className="flex h-full items-end gap-2">{[35,48,44,62,58,72,68,80,76,88,84,92].map((h,i)=><div key={i} className="flex-1 rounded-t bg-accent/70" style={{height:`${h}%`}} />)}</div></div><p className="mt-3 text-xs text-muted-foreground">Visual trend placeholder — live search/analytics connection will populate this chart in Phase 2.</p></section><section className="surface-panel p-5"><h3 className="font-semibold">Priority Issues</h3><div className="mt-4 space-y-2">{["Missing meta descriptions","Broken internal links","Pages without canonical","Slow pages"].map(x=><div key={x} className="flex items-center justify-between rounded-lg border border-border p-3 text-sm"><span>{x}</span><AlertTriangle className="size-4 text-amber-500" /></div>)}</div></section></div>}
    {section==="keywords" && <section className="surface-panel p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">Tracked Keywords</h3><p className="text-xs text-muted-foreground">Add keywords now; rankings connect later.</p></div><button type="button" onClick={()=>toast.info("Keyword storage connects in Phase 2.")} className="inline-flex items-center gap-2 rounded-md bg-accent px-3 py-2 text-xs font-semibold text-accent-foreground"><Plus className="size-4"/>Add keyword</button></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead><tr className="border-b border-border text-xs text-muted-foreground"><th className="pb-3">Keyword</th><th className="pb-3">Position</th><th className="pb-3">Change</th><th className="pb-3">Landing page</th><th className="pb-3">Status</th></tr></thead><tbody>{["construction cost calculator","rebar weight calculator","BBS calculator","cement quantity calculator"].map(k=><tr key={k} className="border-b border-border/70"><td className="py-3 font-medium">{k}</td><td className="py-3">—</td><td className="py-3">—</td><td className="py-3 text-muted-foreground">—</td><td className="py-3"><StatusBadge status="Not connected"/></td></tr>)}</tbody></table></div></section>}
    {section==="content-quality" && <div className="grid gap-4 lg:grid-cols-3">{[["Title & headings","Check hierarchy and keyword intent"],["Meta coverage","Check title and description"],["Readability","Review length and structure"],["Originality","Flag duplicate/similar content"],["Links","Check internal and external links"],["Media","Check alt text and image coverage"]].map(([a,b])=><section key={a} className="surface-panel p-5"><div className="size-10 rounded-xl bg-accent/10 p-2 text-accent"><CheckCircle2 className="size-6"/></div><h3 className="mt-4 font-semibold">{a}</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">{b}</p><button type="button" onClick={()=>toast.info("Content audit connects in Phase 2.")} className="mt-4 rounded-md border border-border px-3 py-2 text-xs font-semibold">Run check</button></section>)}</div>}
    {section==="internal-links" && <section className="surface-panel p-5"><div className="grid gap-3 md:grid-cols-3"><input className={fieldCls} placeholder="Source URL"/><input className={fieldCls} placeholder="Target URL"/><input className={fieldCls} placeholder="Anchor text"/></div><div className="mt-4 rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">Internal link map and broken-link detection will populate after the crawler is connected.</div></section>}
    {section==="sitemap" && <div className="grid gap-4 lg:grid-cols-2"><section className="surface-panel p-5"><h3 className="font-semibold">Sitemap</h3><div className="mt-4 space-y-4"><label className="flex items-center justify-between rounded-lg border border-border p-4"><span><span className="block text-sm font-semibold">Enable sitemap</span><span className="text-xs text-muted-foreground">Include published pages and tools.</span></span><input type="checkbox" defaultChecked/></label><input className={fieldCls} defaultValue="/sitemap.xml"/><button type="button" onClick={()=>toast.info("Sitemap generation connects in Phase 2.")} className="rounded-md bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground">Validate sitemap</button></div></section><section className="surface-panel p-5"><h3 className="font-semibold">Robots.txt</h3><textarea className={fieldCls+" mt-4 min-h-44 font-mono text-xs"} defaultValue={'User-agent: *\nAllow: /\nSitemap: /sitemap.xml'}/><button type="button" onClick={()=>toast.info("Robots publishing connects in Phase 2.")} className="mt-4 rounded-md border border-border px-4 py-2 text-sm font-semibold">Save robots rules</button></section></div>}
    {section==="redirects" && <div className="grid gap-4 lg:grid-cols-[.75fr_1.25fr]"><section className="surface-panel p-5"><h3 className="font-semibold">Add Redirect</h3><div className="mt-4 space-y-3"><input className={fieldCls} placeholder="Old URL /path"/><input className={fieldCls} placeholder="New URL /new-path"/><select className={fieldCls}><option>301 Permanent</option><option>302 Temporary</option></select><button type="button" onClick={()=>toast.info("Redirect storage connects in Phase 2.")} className="w-full rounded-md bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground">Add redirect</button></div></section><section className="surface-panel p-5"><h3 className="font-semibold">404 Log</h3><div className="mt-4 rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">No live 404 data yet. The log will appear after analytics/edge logging is connected.</div></section></div>}
  </div>;
}
...
export function AdminConsole() {
  ...
  function renderPanel() {
    switch (tab) {
      ...
      case "sitemap":
      case "redirects":
        return <SeoWorkspace section={tab} />;
