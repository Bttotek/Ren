...
/* ============================================================
   ENGINEERING WORKSPACES — PHASE 1 UI ONLY
============================================================ */

function BbsProPanel() {
  const [projectView, setProjectView] = useState<"projects" | "schedule" | "settings">("projects");
  const projects = [
    { name: "Residential Block A", client: "Demo Project", bars: 126, weight: "2.84 t", status: "Ready" },
    { name: "Foundation Package", client: "Sample Client", bars: 84, weight: "1.62 t", status: "Draft" },
  ];
  return (
    <div className="space-y-5">
      <section className="surface-panel p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><div className="text-[10px] font-bold uppercase tracking-wider text-accent">Tools / Engineering</div><h2 className="mt-1 font-display text-2xl font-bold">BBS Pro Workspace</h2><p className="mt-1 text-sm text-muted-foreground">Manage reinforcement projects, bar schedules and steel quantities.</p></div>
          <button type="button" className="inline-flex items-center gap-2 rounded-md bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground"><Plus className="size-4" />New Project</button>
        </div>
        <div className="mt-5 flex flex-wrap gap-2">
          {[["projects","Projects"],["schedule","Bar Schedule"],["settings","BBS Settings"]].map(([id,label]) => <button key={id} type="button" onClick={() => setProjectView(id as typeof projectView)} className={"rounded-md px-3 py-2 text-sm font-medium " + (projectView===id ? "bg-secondary text-foreground" : "text-muted-foreground hover:bg-secondary/60")}>{label}</button>)}
        </div>
      </section>
      {projectView === "projects" && <>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[['Projects','2'],['Bar Items','210'],['Steel Weight','4.46 t'],['Exports','18']].map(([label,value]) => <div key={label} className="surface-panel p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{label}</div><div className="mt-2 text-2xl font-bold">{value}</div></div>)}
        </div>
        <div className="surface-panel overflow-hidden">
          <div className="border-b border-border p-4 font-semibold">Recent BBS Projects</div>
          <div className="divide-y divide-border">{projects.map(p => <div key={p.name} className="grid gap-3 p-4 md:grid-cols-[1.4fr_1fr_100px_100px_100px] md:items-center"><div><div className="font-semibold">{p.name}</div><div className="text-xs text-muted-foreground">{p.client}</div></div><div className="text-sm text-muted-foreground">{p.bars} bar items</div><div className="text-sm">{p.weight}</div><div><span className="rounded-full bg-secondary px-2 py-1 text-xs">{p.status}</span></div><button className="rounded-md border border-border px-3 py-2 text-xs font-semibold hover:bg-secondary">Open</button></div>)}</div>
        </div>
      </>}
      {projectView === "schedule" && <div className="surface-panel p-5"><h3 className="font-semibold">Bar Schedule</h3><p className="mt-1 text-sm text-muted-foreground">A dedicated BBS schedule workspace for bar mark, diameter, shape, length, quantity and weight.</p><div className="mt-5 overflow-auto rounded-lg border border-border"><table className="w-full min-w-[720px] text-sm"><thead className="bg-muted/30"><tr>{['Bar Mark','Dia.','Shape','Length','Qty','Weight','Action'].map(x=><th key={x} className="px-3 py-3 text-left text-xs uppercase tracking-wide text-muted-foreground">{x}</th>)}</tr></thead><tbody><tr className="border-t border-border"><td className="px-3 py-3 font-semibold">B01</td><td className="px-3 py-3">16 mm</td><td className="px-3 py-3">Straight</td><td className="px-3 py-3">6.20 m</td><td className="px-3 py-3">24</td><td className="px-3 py-3">23.75 kg</td><td className="px-3 py-3"><button className="rounded border px-2 py-1 text-xs">Edit</button></td></tr></tbody></table></div></div>}
      {projectView === "settings" && <div className="grid gap-4 lg:grid-cols-2"><div className="surface-panel p-5"><h3 className="font-semibold">Calculation Defaults</h3><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="grid gap-1"><span className={labelCls}>Unit-weight formula</span><input className={fieldCls} defaultValue="d² / 162" /></label><label className="grid gap-1"><span className={labelCls}>Default cover (mm)</span><input className={fieldCls} type="number" defaultValue="40" /></label></div></div><div className="surface-panel p-5"><h3 className="font-semibold">Export Defaults</h3><div className="mt-4 grid gap-4"><label className="grid gap-1"><span className={labelCls}>Default format</span><select className={fieldCls} defaultValue="xlsx"><option value="xlsx">Excel (.xlsx)</option><option value="pdf">PDF</option></select></label><label className="grid gap-1"><span className={labelCls}>Include summary</span><select className={fieldCls} defaultValue="yes"><option value="yes">Yes</option><option value="no">No</option></select></label></div></div></div>}
    </div>
  );
}
...
function MeasurementPanel() {
  ...
  const tools = [{name:"Length",unit:"m / ft / in",status:"Active"},{name:"Area",unit:"m² / ft²",status:"Active"},{name:"Volume",unit:"m³ / ft³",status:"Active"},{name:"Weight",unit:"kg / tonne",status:"Active"}];
  return <div className="space-y-5"><section className="surface-panel p-5"><div className="text-[10px] font-bold uppercase tracking-wider text-accent">Tools / Engineering</div><h2 className="mt-1 font-display text-2xl font-bold">Measurement Workspace</h2><p className="mt-1 text-sm text-muted-foreground">Configure measurement tools, units and result precision.</p></section><div className="grid gap-4 lg:grid-cols-[1fr_360px]"><div className="surface-panel overflow-hidden"><div className="border-b border-border p-4 font-semibold">Measurement Tools</div><div className="divide-y divide-border">{tools.map(t=><div key={t.name} className="flex items-center justify-between gap-3 p-4"><div><div className="font-semibold">{t.name}</div><div className="text-xs text-muted-foreground">Supported: {t.unit}</div></div><div className="flex items-center gap-2"><span className="rounded-full bg-secondary px-2 py-1 text-xs">{t.status}</span><button className="rounded-md border border-border px-3 py-2 text-xs font-semibold hover:bg-secondary">Configure</button></div></div>)}</div></div><div className="surface-panel p-5"><h3 className="font-semibold">Unit Defaults</h3><div className="mt-4 grid gap-4"><label className="grid gap-1"><span className={labelCls}>Default unit</span><select className={fieldCls} value={unit} onChange={e=>setUnit(e.target.value)}><option>m</option><option>ft</option><option>mm</option><option>in</option></select></label><label className="grid gap-1"><span className={labelCls}>Decimal precision</span><input className={fieldCls} type="number" min="0" max="6" value={precision} onChange={e=>setPrecision(e.target.value)} /></label><div className="rounded-lg bg-secondary/50 p-4 text-sm">Preview: <strong>12.35 {unit}</strong></div></div></div></div></div>;
}

function LiveSheetPanel() {
  const [view, setView] = useState<"templates" | "builder" | "exports">("templates");
  const templates = [{name:"Quantity Estimate",columns:8,status:"Published"},{name:"BBS Summary",columns:12,status:"Draft"},{name:"Material Takeoff",columns:10,status:"Published"}];
  return <div className="space-y-5"><section className="surface-panel p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-[10px] font-bold uppercase tracking-wider text-accent">Tools / Engineering</div><h2 className="mt-1 font-display text-2xl font-bold">Live Sheet Builder</h2><p className="mt-1 text-sm text-muted-foreground">Create spreadsheet templates, configure columns and manage exports.</p></div><button className="inline-flex items-center gap-2 rounded-md bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground"><Plus className="size-4" />New Template</button></div><div className="mt-5 flex flex-wrap gap-2">{[["templates","Templates"],["builder","Sheet Builder"],["exports","Exports"]].map(([id,label])=><button key={id} onClick={()=>setView(id as typeof view)} className={"rounded-md px-3 py-2 text-sm font-medium " + (view===id ? "bg-secondary" : "text-muted-foreground hover:bg-secondary/60")}>{label}</button>)}</div></section>{view==="templates"&&<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{templates.map(t=><div key={t.name} className="surface-panel p-5"><div className="flex items-start justify-between"><div className="grid size-10 place-items-center rounded-lg bg-secondary"><FileText className="size-5" /></div><span className="rounded-full bg-secondary px-2 py-1 text-xs">{t.status}</span></div><h3 className="mt-4 font-semibold">{t.name}</h3><p className="mt-1 text-sm text-muted-foreground">{t.columns} configured columns</p><div className="mt-4 flex gap-2"><button className="rounded-md border border-border px-3 py-2 text-xs font-semibold hover:bg-secondary">Edit</button><button className="rounded-md border border-border px-3 py-2 text-xs font-semibold hover:bg-secondary">Duplicate</button></div></div>)}</div>}{view==="builder"&&<div className="surface-panel p-5"><h3 className="font-semibold">Sheet Builder</h3><div className="mt-4 overflow-auto rounded-lg border border-border"><table className="w-full min-w-[760px] text-sm"><thead className="bg-muted/30"><tr>{['#','Column','Type','Formula','Required','Action'].map(x=><th key={x} className="px-3 py-3 text-left text-xs uppercase tracking-wide text-muted-foreground">{x}</th>)}</tr></thead><tbody>{['Description','Quantity','Unit','Rate','Amount'].map((x,i)=><tr key={x} className="border-t border-border"><td className="px-3 py-3">{i+1}</td><td className="px-3 py-3 font-medium">{x}</td><td className="px-3 py-3">{i===4?'Formula':'Text'}</td><td className="px-3 py-3 text-xs text-muted-foreground">{i===4?'Quantity × Rate':'—'}</td><td className="px-3 py-3">{i>0?'Yes':'No'}</td><td className="px-3 py-3"><button className="rounded border px-2 py-1 text-xs">Edit</button></td></tr>)}</tbody></table></div></div>}{view==="exports"&&<div className="grid gap-4 md:grid-cols-3">{['Excel (.xlsx)','CSV','PDF'].map(x=><div key={x} className="surface-panel p-5"><div className="font-semibold">{x}</div><p className="mt-1 text-sm text-muted-foreground">Configure export rules, filename pattern and included columns.</p><button className="mt-4 rounded-md border border-border px-3 py-2 text-xs font-semibold hover:bg-secondary">Configure</button></div>)}</div>}</div>;
}
