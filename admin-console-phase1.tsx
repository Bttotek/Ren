...
function DashboardCount({ label, table }: { label: string; table: "profiles" | "blog_posts" | "leads" | "tool_reviews" | "blog_comments" }) {
  ...
  return <div className="surface-panel p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{label}</div><div className="mt-2 text-2xl font-bold">{isLoading ? "—" : data}</div></div>;
}
    ...
    <div className="space-y-6">
      <section className="surface-panel overflow-hidden p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-accent">Operations</div>
            <h2 className="mt-1 font-display text-2xl font-bold">BTTOTEK Master Control</h2>
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Live overview of users, content, leads, reviews and calculator inventory. Each module below has its own workflow instead of a one-size-fits-all editor.</p>
          </div>
          <button type="button" onClick={() => void qc.invalidateQueries({ queryKey: ["admin-dashboard"] })} className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-2 text-xs font-semibold hover:bg-secondary"><RefreshCw className="size-3.5" /> Refresh</button>
        </div>
      </section>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <div className="surface-panel p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Calculators</div><div className="mt-2 text-2xl font-bold">{TOOLS.length}</div></div>
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="surface-panel p-5"><div className="flex items-center gap-2"><CheckCircle2 className="size-4 text-emerald-500" /><h3 className="font-semibold">Security model</h3></div><p className="mt-2 text-sm text-muted-foreground">Admin access is checked before the console loads and database writes are protected by Supabase RLS/admin role policies.</p></div>
        <div className="surface-panel p-5"><div className="flex items-center gap-2"><AlertTriangle className="size-4 text-amber-500" /><h3 className="font-semibold">Download controls</h3></div><p className="mt-2 text-sm text-muted-foreground">PDF and Excel availability, free limits and premium mode are controlled from Download Services.</p></div>
      </div>
    </div>
  );
}
